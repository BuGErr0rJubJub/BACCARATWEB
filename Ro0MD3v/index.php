...
<?php 
function getwin($user){
$q = query("SELECT * FROM statistic WHERE user = ? AND result = 'win'",array($user));
return $q->rowCount();
}
function getlose($user){
$q = query("SELECT * FROM statistic WHERE user = ? AND result = 'lose'",array($user));
return $q->rowCount();
}
?>
  ...
  <nav class="navbar navbar-expand-lg navbar-dark fixed-bottom p-1 font14">
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent-333">
      ...
      <li class="nav-item">
        ...
        <li class="nav-item">
          <a style="margin-right: 1em;" href="?page=m_formula">
